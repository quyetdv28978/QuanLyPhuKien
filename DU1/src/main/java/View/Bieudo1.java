...
public List<Object[]> getA(){
 return DBConnection.getsetFactory().openSession().createQuery(
         "select c.soluong*c.donGia,month(c.hd.ngayTT) from ChiTietHoaDon c where c.trangThai=0 ").getResultList();
    }
...
//                         }else tongTien += ((double)thognKe.get(j)[1]);
...
//
        ...
        if (listItem != null) {
            for (Object[] x : listItem) {
                dataset.addValue((Float)x[0], "Tổng tiền",(Integer)x[1] );
            }
        }
