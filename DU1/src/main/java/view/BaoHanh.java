...
import respon.ChiTietBaoHanhRes;
...
//        jText.add(cbbTenSP);
//        jText.add(txtMoTaSP);
        jText.add(cbbSuaChua);
        jText.add(cbbTrangThai);
...
//        chay1();
    ...
    public void loadTableCTBH() {
        if (chiTietBaoHanhRes.getAll("") != null) {
        dtm = (DefaultTableModel) tbhHienThiCTBH.getModel();
        dtm.setRowCount(0);
        for (ChiTietBaoHanh object : chiTietBaoHanhRes.getAll("")) {
            dtm.addRow(object.toRowCTBH());
        }
        }
    }
    ...
    public void fillFormBH(int row) throws ParseException {
        domaimodel.ChiTietBaoHanh ctkmvm = chiTietBaoHanhRes.getAll("").get(row);
        txtSDT.setText(ctkmvm.getBh().getKh().getSdt());
        lbMaBH.setText(ctkmvm.getBh().getMa());
        lbTenSP.setText(ctkmvm.getBh().getSp().getTenSanPham());
        txtTenKH.setText(ctkmvm.getBh().getKh().getTen());
        ...
        if (ctkmvm.getNgayTra() != null) {
            ...
            int tt = ctkmvm.getBh().getTinhTrang();
    ...
    public ChiTietBaoHanh getDataCTBH(String dk) {
        
        if (dk.equalsIgnoreCase("update")) {
            return new ChiTietBaoHanh(tbhHienThiCTBH.getValueAt(tbhHienThiCTBH.getSelectedRow(), 0).toString(),
                    ...
                    dateNgayTra.getDate(), ((ChiTietBaoHanh) chiTietBaoHanhRes.getAll("").get(tbhHienThiCTBH.getSelectedRow())).getBh()
            );
        }
        KhachHang kh = new KhachHang(tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 4).toString(),
                tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 5).toString());
        SanPham pham = new SanPham(tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 2).toString(),
                tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 3).toString());
        domaimodel.BaoHanh bh = new domaimodel.BaoHanh(tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 0)
                .toString(),
                tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 1).toString(),
                new Date(),
                pham, kh);
        System.out.println("hhh" + chiTietBaoHanhRes.themCTBH(tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 0).
                toString()));
        int a = chiTietBaoHanhRes.themCTBH(tbHienThi.getValueAt(tbHienThi.getSelectedRow(), 0).toString());
        return new ChiTietBaoHanh(jcheck.createID().toString(), jcheck.randomMA(),
                a,
                cbbTrangThai.getSelectedIndex() == 0 ? 0 : 1,
                bh);
    ...
    private boolean checkMaBH(String ma) {
        int count = 0;
        for (int i = 0; i < chiTietBaoHanhRes.getAll("").size(); i++) {
            if (chiTietBaoHanhRes.getAll("").get(i).getBh().getMa().equalsIgnoreCase(ma)) {
                count++;
                if (tbhHienThiCTBH.getSelectedRow() == i) {
                    continue;
                }
                tbhHienThiCTBH.setValueAt(count, i, 10);
                return false;
    ...
    private void initComponents() {
        ...
        tbhHienThiCTBH.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Mã", "Mã BH", "Tên KH", "Sdt", "Mã SP", "Tên SP", "Sửa Chữa", "Ngày Nhận", "Ngày Trả", "Số lượng", "Số lần", "Trạng Thái", "Mô Tả"
            }
        ) {
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(jPanel6, javax.swing.GroupLayout.PREFERRED_SIZE, 910, javax.swing.GroupLayout.PREFERRED_SIZE)
...
//                JOptionPane.showConfirmDialog(this, c.getSoLanBH());
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        if (jcheck.checkClcick(tbhHienThiCTBH, this) == false) {
            ...
            if (a == JOptionPane.YES_OPTION) {
                chiTietBaoHanhRes.update(getDataCTBH("update"));
                System.out.println("ok la");
